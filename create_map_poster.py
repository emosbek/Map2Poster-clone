...
def parse_markers(marker_args):
    """
    Parse marker arguments in the form "lat,lon" into a list of tuples.
    """
    if not marker_args:
        return []
    markers = []
    for raw in marker_args:
        try:
            ...
            raise ValueError(f"Invalid marker '{raw}'. Use 'lat,lon'. ({e})")
    return markers


def create_poster(city, country, point, dist, output_file, output_format, country_label=None, city_label=None, markers=None):
    ...
    # Plot custom markers (project to graph CRS)
    if markers:
        from shapely.geometry import Point
        markers_gdf = GeoDataFrame(geometry=[Point(lon, lat) for lat, lon in markers], crs="EPSG:4326")
        markers_gdf = markers_gdf.to_crs(G_proj.graph["crs"])
        xs = markers_gdf.geometry.x
        ys = markers_gdf.geometry.y
        ax.scatter(xs, ys, s=50, c=THEME["marker"], edgecolors="none", zorder=11)
    ...
    display_city = city_label if city_label is not None else city
...
Examples:
  ...
  python create_map_poster.py --latitude 59.9139 --longitude 10.7522 -t noir -d 8000
  
  # Add custom markers (multiple -m)
  python create_map_poster.py --latitude 59.9139 --longitude 10.7522 -m 59.9145,10.7521 -m 59.9120,10.7500

Options:
  --city, -c        City name (required unless using --latitude/--longitude)
  --country, -C     Country name (required unless using --latitude/--longitude)
  --latitude, --lat Latitude in decimal degrees
  --longitude, --lon Longitude in decimal degrees
  --marker, -m      Add custom marker as "lat,lon" (repeatable)
...
Examples:
  ...
  python create_map_poster.py --list-themes
    ...
    parser.add_argument('--marker', '-m', action='append', help='Add custom marker as "lat,lon" (repeatable)')
    ...
    parser.add_argument('--format', '-f', default='svg', choices=['png', 'svg', 'pdf'],help='Output format for the poster (default: svg)')
    ...
    has_coords = args.latitude is not None or args.longitude is not None
    if has_coords and (args.latitude is None or args.longitude is None):
        print("Error: --latitude and --longitude must be provided together.\n")
        print_examples()
        sys.exit(1)
    if not has_coords and (not args.city or not args.country):
        print("Error: --city and --country are required unless using --latitude/--longitude.\n")
        print_examples()
        sys.exit(1)
    ...
    try:
        ...
        if has_coords:
            ...
            display_city = args.city_label or args.city or "Location"
            display_country = args.country_label or args.country or ""
            output_city = args.city_label or args.city or "location"
        else:
            coords = get_coordinates(args.city, args.country)
            display_city = args.city_label or args.city
            display_country = args.country_label or args.country
            output_city = args.city_label or args.city
        for theme_name in themes_to_generate:
            THEME = load_theme(theme_name)
            output_file = generate_output_filename(output_city, theme_name, args.format)
            create_poster(display_city, display_country, coords, args.distance, output_file, args.format, markers=markers)
